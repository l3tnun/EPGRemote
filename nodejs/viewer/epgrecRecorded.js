...
module.exports = function(response, programs, programCnt, pageNum) {
    ...
    programs.forEach(function(program) {
        programStr += `<li data-icon="action"><a href="javascript:getVideoList('${program.id}', 'stream');" target="_self">`
        ...
        programStr += `</a><a id="actionId${actionId}" href="#" programId="${program.id}" programTitle="${program.title}" programInfo="${program.info}" programChannelName="${program.channel_name}" programDescription="${program.description}" programThumbs="${program.thumbs}" downloadLink="javascript:getVideoList('${program.id}', 'download');" class="openActionMenu"></a></li>`
        actionId += 1;
    });
